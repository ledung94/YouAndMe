    ...
    stages {
        ...
        stage('Deploy MySQL to DEV') {
            steps {
                ...
                sh "docker run --name mysql --rm --network my-dev-network -v mysql-data:/var/lib/mysql -e MYSQL_ROOT_PASSWORD=${MYSQL_ROOT_LOGIN_PSW} -e MYSQL_DATABASE=db_example  -d mysql"
                sh 'sleep 20'
